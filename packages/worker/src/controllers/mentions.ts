...
export async function createMentions(
  ...
  for (const mention of mentions) {
    if (mention.type === 'all') {
      // Get all members of the channel (if group chat)
      if (contextType === 'group_chat') {
        const { results: members } = await db.prepare(`
          SELECT staff_id
          FROM group_chat_members
          WHERE channel_id = ? AND is_active = 1 AND staff_id != ?
        `).bind(contextId, mentioningStaffId).all();
        ...
        for (const member of members) {
          ...
          await db.prepare(`
            INSERT INTO mentions (
              ...
              ticket_id, ticket_number, customer_name
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(
            mentionId,
            messageId,
      ...
      await db.prepare(`
        INSERT INTO mentions (
          id, message_id, mentioned_staff_id, mentioning_staff_id,
          mention_type, context_type, context_id,
          ai_status, ticket_id, ticket_number, customer_name
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(
        mentionId,
        messageId,
