...
export default function TicketDetailPage() {
  ...
  const messages = ticketData?.messages || [] // Chronological order - oldest first, newest at bottom
  ...
  // Auto-scroll to bottom when ticket loads to show latest message and AI Draft
  useEffect(() => {
  ...
  // Backend handles email signature - no need to load it here
  ...
  const handleSendReply = async () => {
    ...
    setIsSending(true)
    try {
      // Send plain text only - backend will append signature when sending email
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto" style={{ minHeight: 0 }}>
          ...
          <div className="p-4 space-y-4 bg-gray-50">
            ...
            {/* Scroll anchor - auto-scroll here when ticket opens */}
              ...
              {showInternalNotes && (
                <div className="flex flex-col bg-yellow-50" style={{ height: `${staffNotesHeight}px` }}>
                      ...
                      {notes.map((note: any) => (
                        <div key={note.id} className="bg-yellow-100 rounded-lg p-3 border border-yellow-200">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center gap-2">
                              {/* Edit/Delete buttons for note owner or admin */}
                              {(note.staff_id === user?.id || user?.role === 'admin') && editingNoteId !== note.id && (
                                <>
                                  <button
                                    onClick={() => {
                                    ...
                                    className="text-xs px-2 py-1 text-gray-600 hover:text-blue-600 hover:bg-white rounded transition-colors"
                                    title="Edit note"
                                  >
                                    Edit
                                  </button>
                                  <button
                                    ...
                                    className="text-xs px-2 py-1 text-gray-600 hover:text-red-600 hover:bg-white rounded transition-colors"
                                    title="Delete note"
                                  >
                                    Delete
                                  </button>
                          ...
                          {editingNoteId === note.id ? (
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={async () => {
                                    if (!ticket || !editingNoteContent.trim()) return;
                                    try {
                                      await ticketsApi.editNote(ticket.ticket_id, note.id, editingNoteContent);
                                      ...
                                      alert(`Failed to edit note: ${error.response?.data?.error || error.message || 'Unknown error'}`);
                                    }
                                  }}
                          ...
                          {note.tags && (
                            <div className="mt-2 flex flex-wrap gap-2">
                              {parseTagsFromStorage(note.tags).map((tag, index) => (
                                <Link
                                  key={index}
                                  to={`/tags?search=${encodeURIComponent(tag)}`}
                        ...
                        <div className="flex items-center gap-3 justify-end">
                          ...
                          <button
                            onClick={async () => {
                              if (!ticket) return;
                              try {
                                await ticketsApi.deleteNote(ticket.ticket_id, deleteConfirmNoteId);
                                setDeleteConfirmNoteId(null);
                                // Force immediate refetch
                                queryClient.invalidateQueries({ queryKey: ['ticket', id] });
                              } catch (error: any) {
                                console.error('Failed to delete note:', error);
                                alert(`Failed to delete note: ${error.response?.data?.error || error.message || 'Unknown error'}`);
                              }
                            }}
                  ...
                  <textarea
                    ...
                    placeholder="Add internal notes... Use @to mention staff, #keyword to add tags (Press Enter to save, Shift+Enter for new line)"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Are you sure you want to delete ticket <span className="font-medium">{formatTicketNumber(ticket.ticket_number)}</span>? 
      ...
      {showMergeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4 max-h-[80vh] overflow-hidden flex flex-col">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Select a ticket to merge into <span className="font-medium">{formatTicketNumber(ticket.ticket_number)}</span> (primary).
              <br />
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-sm text-gray-900">{formatTicketNumber(t.ticket_number)}</span>
              ...
              >
                {isMerging ? (
                  ...
                  <>
                    ...
                    Merge into {formatTicketNumber(ticket.ticket_number)}
                  </>
                )}
              </button>
            </div>
