 ...
 * Archive a mention
 */
export async function archiveMention(c: Context<{ Bindings: Env }>) {
  try {
    const user = c.get('user');
    if (!user) {
      return c.json({ success: false, message: 'Unauthorized' }, 401);
    ...
    // Check if mention belongs to user
    const mention: any = await c.env.DB.prepare(`
      SELECT * FROM mentions WHERE id = ? AND mentioned_staff_id = ?
    ...
    // Only archive Read mentions
    if (mention.is_read === 0) {
      return c.json({ success: false, message: 'Can only archive Read mentions' }, 400);
    }

    // Archive the mention
    await c.env.DB.prepare(`
      UPDATE mentions SET is_archived = 1, archived_at = ? WHERE id = ?
    `).bind(now, mentionId).run();

    return c.json({ success: true, message: 'Mention archived' });
  } catch (error) {
    console.error('[Mentions] Error archiving mention:', error);
    return c.json({ success: false, message: 'Failed to archive mention' }, 500);
  }
}
