  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        s.last_name,
        s.email,
        s.avatar_url as profile_picture
      FROM group_chat_messages m
...
export async function sendMessage(c: Context<{ Bindings: Env }>) {
  try {
    ...
    const channelId = c.req.param('id');
    const { content, attachments, tags } = await c.req.json();
    ...
    console.log('[Group Chat] Message content:', content);
    const mentions = parseMentions(content?.trim() || '');
    console.log('[Group Chat] Parsed mentions:', mentions.length, mentions);
    if (mentions.length > 0) {
      console.log('[Group Chat] Creating mentions for channel:', channelId, 'user:', user.id);
      ...
      console.log('[Group Chat] Mentions created successfully');
...
export async function editMessage(c: Context<{ Bindings: Env }>) {
  try {
    ...
    // Update message content and optionally remove attachment
    if (deleteAttachment) {
      await c.env.DB.prepare(`
        UPDATE group_chat_messages 
        ...
        WHERE id = ?
      `).bind(content.trim(), nowISO, user.id, nowISO, messageId).run();
    } else {
      await c.env.DB.prepare(`
        UPDATE group_chat_messages 
        SET content = ?, edited_at = ?, edited_by = ?, updated_at = ?
...
export async function deleteMessage(c: Context<{ Bindings: Env }>) {
  try {
    ...
    await c.env.DB.prepare(`
      ...
      WHERE id = ?
    `).bind(nowISO, user.id, nowISO, messageId).run();
...
/**
 * GET /api/group-chat/settings/auto-archive
 * Get the auto-archive hours setting for mentions
 */
export async function getAutoArchiveHours(c: Context<{ Bindings: Env }>) {
  try {
    const user = c.get('user');
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
    ...
    // Get from KV storage, default to 12 hours
    const hoursStr = await c.env.APP_CONFIG?.get('mentions_auto_archive_hours');
    const hours = hoursStr ? parseInt(hoursStr, 10) : 12;

    return c.json({ hours });
  } catch (error) {
    console.error('[Group Chat] Error getting auto-archive hours:', error);
    return c.json({ error: 'Failed to get auto-archive hours' }, 500);
  }
}

/**
 * PUT /api/group-chat/settings/auto-archive
 * Set the auto-archive hours setting for mentions (admin only)
 */
export async function setAutoArchiveHours(c: Context<{ Bindings: Env }>) {
  try {
    const user = c.get('user');
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
    ...
    // Validate hours (must be a number, 0 means never archive)
    if (typeof hours !== 'number' || hours < 0) {
      return c.json({ error: 'Invalid hours. Must be a non-negative number.' }, 400);
    }

    // Store in KV
    await c.env.APP_CONFIG?.put('mentions_auto_archive_hours', hours.toString());

    return c.json({ success: true, hours });
  } catch (error) {
    console.error('[Group Chat] Error setting auto-archive hours:', error);
    return c.json({ error: 'Failed to set auto-archive hours' }, 500);
  }
}
